...
with open("Tablas_en_csv/Aportes_Privados_2017_candidato.csv", "w") as document:
    fila = 0
    print("ID,NOMBRE APORTANTE, TIPO APORTANTE, CANDIDATURA ID, TIPO APORTE, FECHA, MONTO", file=document)
    for line in lines[1::]:
        #print(line)
        line = line.strip().split(",")
        if (line[2] == "CARLOS GUAJARDO DIAZ"):
            continue
        if (line[2] == "RICARDO COGLER GALINDO"):
            line[2] = "NICOLAS COGLER GALINDO"
        if (line[2] == "ROBERTO PAREDES SANDOVAL"):
            line[2] = "RICARDO PAREDES SANDOVAL"
        if (line[2] == "HUGO PARDO VALLEJOS"):
            line[2] = "CESAR PARDO VALLEJOS"
        #partido_id = partidos[clean_partido(line[6])]
        ano = line[8].split("-")[2]
        territorio_id = territorios[clean_territorio(line[5])]
        candidato_id = buscar_id_candidato(line[2], candidatos)
        cargo = line[4].replace("PRESIDENTE", "PRESIDENCIAL")
        # NOTA el arhicvo de aportes tien mal los partidos
        candidatura_id = candidaturas[(cargo, territorio_id, candidato_id, ano)]
        print(str(fila) + "," + line[0] + "," + line[1] + "," + candidatura_id + "," + line[7] + "," + line[8] + "," + line[9], file=document)
        fila += 1
